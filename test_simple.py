...
def test(model, lpips_func, args, device, iter):
...
if __name__ == '__main__':
    ...
    psnrs_list = []
    args.start_iter = 0
    for ckpt in os.listdir(args.ckpt):
        if 'pth' in ckpt:
            curr_ckpt = os.path.join(args.ckpt,ckpt)
            ...
            lpips_value, psnr_value = test(g_ema, lpips_func, args, device, 10000)
            print(f'Results with {curr_ckpt} - lpips_value: {lpips_value} - psnr_value: {psnr_value}')
            lpips_list.append(lpips_value.cpu().squeeze().item())
            psnrs_list.append(psnr_value.cpu().squeeze().item())
    fig, axs = plt.subplots(1, 2, figsize=(20, 10))
    axs[0].plot(lpips_list, label='test lpips')
    axs[1].plot(psnrs_list, label='test psnr')
    axs[0].set_xlabel('Checkpoints')
    axs[0].set_ylabel('lpips')
    axs[1].set_xlabel('Checkpoints')
    axs[1].set_ylabel('psnr')
    plt.legend()
    fig.savefig('lpips_psnr_test.jpg')
