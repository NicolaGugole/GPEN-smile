...
import __init_paths
from training.data_loader.dataset_face_classic import FaceDataset
from face_model.gpen_model_classic import FullGenerator, Discriminator
...
def validation(model, lpips_func, args, device, iter):
    lq_files = [os.path.join(args.in_path,i)[:-1] for i in open(args.val_files) if 'jpg' in i] # remove '\n' at the end
    hq_files = [os.path.join(args.out_path,i)[:-1] for i in open(args.val_files) if 'jpg' in i] # remove '\n' at the end
...
def train(args, loader, generator, discriminator, losses, g_optim, d_optim, g_ema, lpips_func, device, heatmap_blur=None):
    if get_rank() == 0:
        wandb.init(project='lifting-gpen')
