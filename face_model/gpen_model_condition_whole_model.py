...
class Generator(nn.Module):
    ...
    def forward(
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            skip = self.label_embeddings(skip, label, skip.shape[-1])
...
class LabelEmbedding(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, label: Optional[torch.LongTensor], embedding_size: int) -> torch.Tensor:
        if label is not None:
            return inputs * self.embeddings[int(math.log(embedding_size, 2))](label)
        else:
            return inputs * self.identity_embeddings[int(math.log(embedding_size, 2))]
...
class FullGenerator(nn.Module):
    ...
    def forward(self,
        ...
        for i in range(self.log_size-1):
            ...
            inputs = self.generator.label_embeddings(inputs, label, inputs.shape[-1])
            inputs = ecd(inputs)
            noise.append(inputs)
        # multiply last one 
        inputs = self.generator.label_embeddings(inputs, label, inputs.shape[-1])
