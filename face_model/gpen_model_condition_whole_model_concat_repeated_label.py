...
class LabelEmbedding(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, label: Optional[torch.LongTensor]) -> torch.Tensor:
        return torch.cat((inputs, torch.ones_like(inputs)*label), 1)
...
class FullGenerator_SR(nn.Module):
    def __init__(
        ...
        conv = [ConvLayer(6, channels[in_size], 1, device=device)]  # concatenating labels
        self.ecd0 = nn.Sequential(*conv)
...
if __name__ == '__main__':
    ...
    yhat = torch.randn(1,3,1024,1024)
