...
class Generator(nn.Module):
    ...
    def forward(
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            # skip = self.label_embeddings(skip, label)
...
class LabelEmbedding(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, label: Optional[torch.LongTensor]) -> torch.Tensor:
        return nn.functional.pad(inputs, (0,1, 0,1), 'constant', label)
...
class FullGenerator(nn.Module):
    def __init__(
        ...
        for i in range(self.log_size, 2, -1):
            ...
            conv = [ConvLayer(in_channel, out_channel, 3, downsample=True, device=device, stride=2 if i != range(self.log_size, 2, -1)[-1] else 3)] 
...
if __name__ == '__main__':
    fg = FullGenerator(1024, 512, 8, label_num=3)
    yhat = torch.ones(1,3,1024,1024)
    fg(yhat, 10000, label=0)